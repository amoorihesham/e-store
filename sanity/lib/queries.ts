...
export const GET_PRODUCTS_QUERY = defineQuery(`*[_type=='product']{...,"category":category->{...},images[]{
  _key,asset,alt}}`);
export const GET_SEARCHED_PRODUCTS_QUERY =
  defineQuery(`*[_type=='product' && name match $searchTerm]{...,"category":category->{...},images[]{
  _key,asset,alt}}`);
...
export const GET_ORDER_QUERY = defineQuery(`*[_type=='order' && orderNumber == $orderNumber][0]{
    _id,
    orderNumber,
    status,
    totalPrice,
