...
export const GET_PRODUCTS_QUERY = defineQuery(`*[_type=='product']{...,"category":category->{...}}`);
export const GET_SEARCHED_PRODUCTS_QUERY = defineQuery(`*[_type=='product' && name match $searchTerm]{...,"category":category->{_id,title}}`);
export const GET_PRODUCT_QUERY = defineQuery(`*[_type=='product' && slug.current == $productId][0]{...,"category":category->{...}}`);
